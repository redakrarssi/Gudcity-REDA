...
export class BusinessSettingsService {
      ...
      if (settings.name || settings.businessName || settings.phone || settings.email ||
        ...
        await sql`
          UPDATE business_profile SET
            business_name = COALESCE(${nameValue}, business_name),
            phone = COALESCE(${settings.phone}, phone),
            address_line1 = COALESCE(${settings.address}, address_line1),
  ...
  private static combineSettingsData(
    ...
    return {
      ...
      businessName: user?.business_name || profile?.business_name || '',
