...
export async function ensureEnrollmentProcedureExists(): Promise<boolean> {
  try {
    ...
    // Create the function if it doesn't exist (no explicit COMMIT/ROLLBACK inside functions)
    await sql`
      ...
      BEGIN
        BEGIN
          ...
          INSERT INTO customer_notifications (
            id, customer_id, business_id, type, title, message, data, requires_action, action_taken, is_read, created_at
          ) VALUES (
            gen_random_uuid(),
            business_id_val,
            business_id_val,
            ...
            END,
            jsonb_build_object('programId', program_id_val, 'programName', program_name_val, 'customerId', customer_id_val, 'approved', is_approved),
            FALSE,
            FALSE,
            FALSE,
            NOW()
          ...
          INSERT INTO customer_notifications (
            ...
            END,
            jsonb_build_object('programId', program_id_val, 'programName', program_name_val, 'businessName', business_name_val),
            FALSE,
            TRUE,
            FALSE,
            NOW()
          ...
          END IF;
          SELECT EXISTS (
            SELECT 1 FROM program_enrollments WHERE customer_id = customer_id_val AND program_id = program_id_val
          ) INTO enrollment_exists;
          IF enrollment_exists THEN
            UPDATE program_enrollments
            SET status = 'ACTIVE', updated_at = NOW()
            WHERE customer_id = customer_id_val AND program_id = program_id_val AND status != 'ACTIVE'
          ...
          ELSE
            INSERT INTO program_enrollments (
              ...
              customer_id_val, program_id_val, business_id_val, 'ACTIVE', 0, 0, NOW()
            ) RETURNING id INTO enrollment_id_val;
          END IF;
          SELECT EXISTS (
            SELECT 1 FROM loyalty_cards WHERE customer_id = customer_id_val AND program_id = program_id_val
          ) INTO card_exists;
          IF NOT card_exists THEN
            ...
            INSERT INTO loyalty_cards (
              id, customer_id, program_id, business_id, card_number, status, points, card_type, tier, points_multiplier, is_active, created_at, updated_at
            ) VALUES (
              card_id_val, customer_id_val, program_id_val, business_id_val, card_number_val, 'ACTIVE', 0, 'STANDARD', 'STANDARD', 1.0, true, NOW(), NOW()
            );
          ELSE
            SELECT id INTO card_id_val FROM loyalty_cards WHERE customer_id = customer_id_val AND program_id = program_id_val LIMIT 1;
          END IF;
          SELECT id INTO existing_notification FROM customer_notifications 
            WHERE customer_id = customer_id_val AND type = 'CARD_CREATED' AND data->>'programId' = program_id_val LIMIT 1;
          IF existing_notification IS NULL THEN
            INSERT INTO customer_notifications (
              ...
              jsonb_build_object('programId', program_id_val, 'programName', program_name_val, 'businessName', business_name_val, 'cardId', card_id_val),
              FALSE,
              FALSE,
              FALSE,
              NOW()
    ...
    // Also install a maintenance helper to fix any missing cards from past inconsistencies
    await sql`
      ...
      BEGIN
        ...
        SELECT
          gen_random_uuid(), pe.customer_id, pe.program_id, pe.business_id,
          'GC-FIXED-' || to_char(NOW(), 'YYMMDD') || '-' || floor(random() * 10000)::TEXT,
          'ACTIVE', 0, 'STANDARD', 'STANDARD', 1.0, true, NOW(), NOW()
        FROM program_enrollments pe
        LEFT JOIN loyalty_cards lc ON lc.customer_id = pe.customer_id AND lc.program_id = pe.program_id
        WHERE pe.status = 'ACTIVE' AND lc.id IS NULL;
        GET DIAGNOSTICS fixed_cards = ROW_COUNT;
