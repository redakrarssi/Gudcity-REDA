  ...
  if (typeof window !== 'undefined') {
    try {
      ...
      if ((window as any).chrome && (window as any).chrome.runtime) {
        (window as any).chrome.runtime.lastError = null;
      }
      
      console.log('Browser extension error suppression setup complete');
    } catch (error) {
      console.warn('Error setting up browser extension error suppression:', error);
    }
  }
}
