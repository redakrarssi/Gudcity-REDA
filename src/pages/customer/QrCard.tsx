...
const CustomerQrCard = () => {
  ...
  return (
    <CustomerLayout>
      <div className="max-w-2xl mx-auto pb-12 qr-card-page">
        {/* Header */}
        <div className={`transition-all duration-500 ease-out transform ${animateIn ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
            <div className="qr-card-header">
              ...
              <p className="text-gray-500 mt-1 header-subtitle">{t('qrCard.useThisCardToCollect')}</p>
            </div>
          </div>
        </div>
        ...
        {successMessage && (
          <div className="mb-4 bg-green-50 border-l-4 border-green-500 p-4 success-message">
            <div className="flex">
              <div className="flex-shrink-0">
                <CheckCircle2 className="h-5 w-5 text-green-500 success-icon" />
              </div>
              <div className="ml-3">
                <p className="text-sm text-green-700 success-text">{successMessage}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {error && (
          <div className="mb-4 bg-red-50 border-l-4 border-red-500 p-4 error-message">
            <div className="flex">
              <div className="flex-shrink-0">
                <AlertCircle className="h-5 w-5 text-red-500 error-icon" />
              </div>
              <div className="ml-3">
                <p className="text-sm text-red-700 error-text">{error}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className={`mb-8 transition-all duration-500 ease-out transform ${animateIn ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
          <div className="bg-white rounded-xl shadow-lg overflow-hidden qr-card-container">
            <div className="p-6 qr-card-content">
              {user?.id && (
                <QRCard 
            ...
            {/* Card Actions */}
            <div className="px-6 py-3 bg-gray-50 border-t border-gray-100 card-actions">
              <div className="flex flex-wrap justify-center gap-4">
                <button 
                  onClick={handleCopyId} 
                  className="flex items-center text-gray-600 hover:text-blue-600 text-sm transition-colors action-button"
                >
                  <Copy className="h-4 w-4 mr-1 action-icon" />
                ...
                <button 
                  ...
                  className="flex items-center text-gray-600 hover:text-blue-600 text-sm transition-colors action-button"
                >
                  <Share className="h-4 w-4 mr-1 action-icon" />
                  {t('qrCard.shareCard')}
                </button>
                ...
                <button 
                  ...
                  className="flex items-center text-gray-600 hover:text-blue-600 text-sm transition-colors action-button"
                >
                  <Download className="h-4 w-4 mr-1 action-icon" />
                  {t('qrCard.printCard')}
                </button>
              </div>
            </div>
        ...
        {/* Programs Section */}
        <div className={`mb-8 transition-all duration-500 ease-out transform delay-100 programs-section ${animateIn ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
          <h2 className="text-xl font-semibold mb-3 flex items-center programs-title">
            <BadgeCheck className="mr-2 h-5 w-5 text-blue-600 programs-icon" />
            {t('qrCard.myPrograms')}
          </h2>
          ...
          {isLoading ? (
            ...
            <div className="grid gap-4 grid-cols-1 md:grid-cols-2 programs-grid">
              {enrolledPrograms.map((program) => (
                <div 
                  key={program.id} 
                  className="bg-white shadow-md rounded-lg p-4 border border-gray-200 program-card"
                >
                  <div className="flex justify-between items-center program-header">
                    <h3 className="font-medium program-name">{program.programName}</h3>
                    <span className="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded program-tier">
                      {program.tierLevel}
                    </span>
                  </div>
                  <p className="text-sm text-gray-600 mt-1 program-points">
                    {t('qrCard.points')}: {program.points}
                  </p>
                  <p className="text-xs text-gray-500 mt-2 program-joined">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center empty-programs">
              <p className="text-gray-600 mb-4 empty-title">{t('qrCard.youHaveNotJoinedAnyPrograms')}</p>
        ...
        {/* Promos Section */}
        <div className={`mb-8 transition-all duration-500 ease-out transform delay-200 promos-section ${animateIn ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
          <h2 className="text-xl font-semibold mb-3 flex items-center promos-title">
            <Tag className="mr-2 h-5 w-5 text-purple-600 promos-icon" />
          ...
          {isLoading ? (
            ...
            <div className="grid gap-4 grid-cols-1 md:grid-cols-2 promos-grid">
              {availablePromos.map((promo) => (
                <div 
                  key={promo.id} 
                  className="bg-white shadow-md rounded-lg p-4 border border-gray-200 promo-card"
                >
                  <div className="flex justify-between items-center promo-header">
                    <h3 className="font-medium promo-code">{promo.code}</h3>
                    <span className="bg-purple-100 text-purple-800 text-xs font-semibold px-2.5 py-0.5 rounded promo-discount">
                      {promo.discountType === 'PERCENTAGE' ? `${promo.discountValue}% OFF` : `$${promo.discountValue} OFF`}
                    </span>
                  </div>
                  <p className="text-sm text-gray-600 mt-1 promo-description">
                    {promo.description || t('qrCard.specialDiscountOffer')}
                  </p>
                  <div className="flex justify-between items-center mt-4 promo-details">
                    <p className="text-xs text-gray-500 promo-expiry">
                      {t('qrCard.expires')}: {new Date(promo.endDate).toLocaleDateString()}
                    </p>
                    {!promo.claimed ? (
                      <button
                        onClick={() => handleClaimPromo(promo.id)}
                        className="px-3 py-1 bg-purple-600 text-white rounded-md text-sm promo-claim"
                        disabled={isLoading}
                      >
                        {t('qrCard.claim')}
                      </button>
