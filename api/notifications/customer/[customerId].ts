import type { VercelRequest, VercelResponse } from '@vercel/node';
import { authenticateRequest } from '../../_lib/auth';
import { formatSuccessResponse, formatErrorResponse } from '../../_services/responseFormatter';
import sql from '../../_lib/db';

export default async function handler(req: VercelRequest, res: VercelResponse) {
  // Only allow GET requests
  if (req.method !== 'GET') {
    return res.status(405).json(formatErrorResponse('Method not allowed', 405));
  }

  try {
    // Authenticate the request
    const authResult = await authenticateRequest(req);
    if (!authResult.authenticated || !authResult.user) {
      return res.status(401).json(formatErrorResponse('Unauthorized', 401));
    }

    const { customerId } = req.query;

    if (!customerId || typeof customerId !== 'string') {
      return res.status(400).json(formatErrorResponse('Customer ID is required', 400));
    }

    const customerIdInt = parseInt(customerId);

    // Users can only see their own notifications unless they're admin or business
    if (
      authResult.user.user_type !== 'admin' && 
      authResult.user.user_type !== 'business' &&
      authResult.user.id !== customerIdInt
    ) {
      return res.status(403).json(formatErrorResponse('Access denied', 403));
    }

    // Get customer notifications
    const notifications = await sql`
      SELECT 
        cn.id::text,
        cn.customer_id::text as "customerId",
        cn.business_id::text as "businessId",
        cn.type,
        cn.title,
        cn.message,
        cn.data,
        cn.reference_id as "referenceId",
        cn.requires_action as "requiresAction",
        cn.action_taken as "actionTaken",
        cn.is_read as "isRead",
        cn.read_at as "readAt",
        cn.created_at as "createdAt",
        cn.updated_at as "updatedAt",
        cn.expires_at as "expiresAt",
        u.name as "businessName"
      FROM customer_notifications cn
      LEFT JOIN users u ON cn.business_id = u.id
      WHERE cn.customer_id = ${customerIdInt}
      ORDER BY cn.created_at DESC
      LIMIT 100
    `;

    return res.status(200).json(formatSuccessResponse({
      notifications,
      count: notifications.length
    }));
  } catch (error) {
    console.error('Error getting customer notifications:', error);
    return res.status(500).json(formatErrorResponse('Internal server error', 500));
  }
}

