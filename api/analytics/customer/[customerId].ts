...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Authorization check - only the customer themselves or admin can view analytics
    if (
      authResult.user.user_type !== 'admin' &&
      authResult.user.id.toString() !== customerId
    ) {
    ...
    const analytics = await AnalyticsServerService.getCustomerAnalytics(
    ...
    console.error('Error in customer analytics endpoint:', error);
